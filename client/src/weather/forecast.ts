...
function buildSummary(rainDays: number, low: number | null, high: number | null): string {
  ...
  return `${details.join(" and ")} — we'll use this to suggest useful extras for your list.`;
}
...
export function getForecastAvailability(stop: Stop, today = new Date()) {
  ...
  return {
    available: startOfDay(stop.startDate) <= maxForecastDate && startOfDay(stop.endDate) <= maxForecastDate,
    availableFrom: addDays(startOfDay(stop.startDate), -(MAX_FORECAST_DAYS - 1)),
    maxForecastDate,
  };
}

export async function fetchTripForecast(stops: Stop[]): Promise<TripForecastResult> {
  const stop = stops[0];
  if (!stop) {
  ...
  if (!response.ok) {
    return { status: "error", message: "Unable to load the forecast right now." };
  }
  ...
  const rainDays = days.filter((day) => day.precipitationMillimeters > 0 || day.weatherCode >= 51).length;
  const averageWindSpeed =
    windSpeeds.length > 0
      ? round(windSpeeds.reduce((total, speed) => total + speed, 0) / windSpeeds.length)
      : round(data.current?.wind_speed_10m);
  const maxUv = round(Math.max(...uvIndexes.filter(Number.isFinite)));
  const extrasCount = [rainDays > 0, low !== null && low <= 5, high !== null && high >= 28, averageWindSpeed !== null && averageWindSpeed >= 30].filter(Boolean).length;
